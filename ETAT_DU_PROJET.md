...
| **Jiraiya** | endan, senpu (feu bouche → proj), rasengan (main) | 🔸 **EN COURS — c'est là qu'on s'est arrêté** |
| **Deidara** | strong, c1, clone_argile, c2 (bombes d'argile) | ⬜ à faire |
| **Saï** | souris/faucon/faucon_plongeant/serpent/lion d'encre (5 projectiles) | ⬜ à faire |
| **Suigetsu** | suiko (prison d'eau) | ⬜ à faire |
| Orochimaru, Pain, Konan, Shikamaru, Karin, Jūgo | *aucun* — leurs effets sont déjà dessinés dans les sprites du corps | ✅ rien à faire |

**Donc : il reste 4 persos à caler (Jiraiya, Deidara, Saï, Suigetsu).**
...
Finir le calage des effets : **Jiraiya** (mesures ci-dessus prêtes), puis **Deidara**, **Saï**, **Suigetsu**.
Boucle : Claude place les `orb`/`ofx` → génère `FICHE_<perso>_orbes.png` → Raph la renvoie → Claude valide/corrige.

**Ensuite (phase 2)** : IA autonome temps réel, hand-tracking caméra (MediaPipe), packaging Electron `.exe`/`.dmg`.
...
> On bosse sur konoha-vivant (github.com/zarentos/konoha-vivant). Les 15 persos sont découpés. On cale la position des effets frame par frame, un perso à la fois, via une fiche PNG que je te renvoie. Faits : Naruto, Sasuke, Itachi, Sakura, Kakashi. En cours : Jiraiya. Restent : Deidara, Saï, Suigetsu. Reprends là.
